      ...
      <div className="w-screen px-5 py-4">
        ...
        {started && (
          <div className="text-center mb-4">
            <p className="text-lg font-bold text-font_primary">Time Remaining</p>
            <p className="text-2xl font-mono text-red-500">{formatTime(timer)}</p>
          </div>
        )}
        ...
        <div className="space-y-4">
          {sampleExercises.map((exercise) => (
            ...
            >
              <div className="flex flex-col">
                <p className="text-base font-medium text-font_primary">{exercise.name}</p>
                <p className="text-sm text-icon">
                  {exercise.sets} sets | {exercise.reps} reps
                </p>
              </div>
              <button onClick={() => toggleComplete(exercise.id)} disabled={!started}>
                {completed.includes(exercise.id) ? (
                  <FaCheckCircle className="w-6 h-6 text-green-500" />
                ) : (
                  <FaRegCircle className="w-6 h-6 text-gray-400" />
