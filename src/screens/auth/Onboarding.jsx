...
export default function Onboarding({ onComplete }) {
  ...
  return (
    <div className="min-h-screen w-screen bg-white flex flex-col items-center justify-between py-10 px-4 font-poppin">
      {/* Image */}
      <img
        src={image}
        alt={title}
        className="object-contain w-full max-h-[50vh] sm:max-h-[60vh] mb-6"
      />

      {/* Title & Content */}
      <div className="text-center max-w-md w-full px-4">
        <h2 className="text-black text-2xl font-bold mb-2">{title}</h2>
        <p className="text-gray-600 text-base sm:text-lg">{content}</p>
      </div>
      ...
      <div className="mt-10 sm:mt-12">
        <button
          onClick={handleNext}
          className="relative w-16 h-16 bg-transparent outline-none focus:outline-none"
        >
          <svg
            className="absolute top-0 left-0 w-full h-full pointer-events-none"
            viewBox="0 0 36 36"
          ...
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-primary text-white rounded-full p-3">
            <FaAngleRight className="w-4 h-4" />
          </div>
        </button>
      </div>
